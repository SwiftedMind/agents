...
public enum Input: Equatable, Hashable, Sendable {
  ...
  public struct ItemList: Equatable, Hashable, Sendable {
  ...
  public enum ListItem: Equatable, Hashable, Sendable {
    /// A message input to the model with a role indicating instruction following hierarchy.
    ///
    /// Instructions given with the `developer` or `system` role take precedence over instructions given with the `user`
    /// role.
    ///
  ...
  /// Text, image, or audio input to the model, used to generate a response. Can also contain previous assistant
  /// responses.
  @Codable @UnTagged
  public enum Content: Equatable, Hashable, Sendable {
    /// Text, image, or audio input to the model, used to generate a response. Can also contain previous assistant
    /// responses.
    @Codable @CodedAt("type") @CodingKeys(.snake_case)
    public enum ContentItem: Equatable, Hashable, Sendable {
      ...
      /// - Parameter imageUrl: The URL of the image to be sent to the model. A fully qualified URL or base64 encoded
      /// image in a data URL.
  ...
  public var listItems: [ListItem] {
    switch self {
    case let .text(text): return [.item(.inputMessage(content: .text(text)))]
    case let .list(items): return items
    }
  }
  
  public mutating func append(_ item: ListItem) {
    switch self {
    case let .text(text):
      self = .list([.item(.inputMessage(content: .text(text)))] + [item])
    case let .list(items):
...
public extension Input.ListItem {
  ...
  /// - Parameter content: Text, image, or audio input to the model, used to generate a response. Can also contain
  ...
  static func fileSearch(
    id: String,
    queries: [String] = [],
    status: Item.FileSearchCall.Status,
    results: [Item.FileSearchCall.Result]? = nil
  ) -> Self {
  ...
  /// - Parameter acknowledgedSafetyChecks: The safety checks reported by the API that have been acknowledged by the
  /// developer.
  ...
  static func functionCall(
    ...
    id: String,
    name: String,
    status: Item.FunctionCall.Status
  ) -> Self {
    .item(Item.Input.functionCall(Item.FunctionCall(
      arguments: arguments,
      callId: callId,
      id: id,
      name: name,
  ...
  static func functionCallOutput(
    id: String? = nil,
    status: Item.FunctionCall.Status? = nil,
    callId: String,
    output: String
  ) -> Self {
    .item(Item.Input.functionCallOutput(Item.FunctionCallOutput(
      id: id,
      status: status,
      callId: callId,
      output: output
  ...
  static func reasoning(
    ...
    status: Item.Reasoning.Status? = nil
  ) -> Self {
